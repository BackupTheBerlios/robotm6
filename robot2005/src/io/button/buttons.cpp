...
// ButtonCL::autoTest
// ----------------------------------------------------------------------------
bool ButtonCL::autoTest(bool *ioStatus)
{
    if (ioStatus != NULL) {
        ioStatus[0] = ! Events->check(evt_);
    }
    // le bouton doit etre relache => donc l'evenement doit etre unraise
    return ! Events->check(evt_);
}
