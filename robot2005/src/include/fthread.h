...
/******** types ****************/
typedef struct ft_event_st *ft_event_t;

typedef struct ft_thread_st *ft_thread_t;

typedef struct ft_scheduler_st *ft_scheduler_t;
...
ft_thread_t    ft_thread_create_unlinked (ft_executable_t runnable,
		                          ft_executable_t cleanup,
		                          void *args);
...
/* scheduler start, reaction, instant */
int  ft_scheduler_start   (ft_scheduler_t);

void ft_scheduler_instant (ft_scheduler_t);

#define INIT_INSTANT   0
#define PROCEED_PHASE  1
#define NEED_NEW_PHASE 2
#define CLOSE_INSTANT  3
int  ft_scheduler_react   (ft_scheduler_t,int k);
...
int ft_thread_is_linked            (ft_thread_t);
...
#define STATE_LINK(num,sched) \
   _STATE(num) _LINK(sched)
...
#define _LINK(sched) \
   if (ft_thread_scheduler () != sched) {\
      _set_thread_is_linked (_self,0);\
      _set_thread_scheduler (_self,sched);\
   }\
   GOTO_NEXT

/****************************************************/
extern int _mutex_locks;  // for debugging
extern void ft_trace_instants (char b);
