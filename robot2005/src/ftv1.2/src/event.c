...
static void* _values_malloc (ft_event_t evt)
{
   ...
   evt->values = (void**)mymalloc (evt->value_max * sizeof (void*));
   return evt->values;
}
...
{
   evt->value_max += VALUES_CHUNK;
   evt->values = (void**)realloc (evt->values,evt->value_max * sizeof (void*));
   return evt->values;
}
...
{
   ft_event_t new_event = (ft_event_t)mymalloc (sizeof (struct ft_event_st));
...
void _register_in_event (ft_event_t event)
{
   ...
   item_cell_t to_register = _append_item_list (event->waiting,cell);
   _set_thread_awaited_event (ft_thread_self (),event);
   _set_thread_event_cell (ft_thread_self (),to_register);
}

void _extract_from_event (ft_event_t event,item_cell_t cell)
{
   _extract (event->waiting,cell);
}
...
static int _awake_one (void *item,void *arg)
{
   ...
   if (!_get_thread_is_suspended (thread)
       && _get_thread_status (thread) == _WAIT)
   {
      _unmask (sched,cell);
      _unregister_in_timer (thread);
       return REMOVE;
   }
   return KEEP;
}
...
void _event_generate (ft_event_t event)
{
   if (!_event_is_generated (event)) {
      ...
      if (_get_scheduler_eoi (sched)) {
	 printf ("LOST EVENT AT INSTANT %ld                      \n",_get_scheduler_instant (sched));fflush(stdout);
	 exit (-1);
      }
