...
{ 
   ...
   _set_thread_deadline (self,delay);
      
   while (1) {
      if (_get_thread_deadline (self) <= 0) return OK;
      _POST_TIMER (sched);
      _release (_WAIT);
   }
}
...
{ 
   ...
   while(1) {
      _CONTROL_TIMEOUT (_get_thread_deadline (self));
      if (_event_is_generated (event)) return OK;
      _POST_TIMER (sched);	 
      _POST_EVENT (event);
      _release (_WAIT);
...
int ft_thread_link (ft_scheduler_t sched)
{
   ...
   res = _start_phase (self); // verifications by it

   if (_get_scheduler_eoi (sched)) ft_thread_cooperate ();
