...
struct ft_thread_st
{
   ...
   ft_event_t             awaited_event;// awaited event, when registered
   item_cell_t            event_cell;   // event cell, when registered
   item_cell_t            timer_cell;   // timer cell, when registered
   int                    deadline;     // time limit, when waiting
...
{
   if (thread->signal) ft_event_deallocate (thread->signal);
...
void _decr_thread_deadline (ft_thread_t thread)
{
   thread->deadline--;
}
...
/*********/
void _set_thread_awaited_event (ft_thread_t thread,ft_event_t event)
{
   thread->awaited_event = event;
}
void _set_thread_event_cell (ft_thread_t thread,item_cell_t cell)
{
   thread->event_cell = cell;
}
void _set_thread_timer_cell (ft_thread_t thread,item_cell_t cell)
{
   thread->timer_cell = cell;
}
...
ft_thread_t _make_thread (ft_executable_t cleanup,void *args)
{
   ...
   ft_thread_t thread = (ft_thread_t)mymalloc (sizeof (struct ft_thread_st));
   ...
   thread->deadline         = -1;
   thread->awaited_event    = NULL;
   thread->event_cell       = NULL;
   thread->timer_cell       = NULL;
...
/*****************************************************/
// creation of unlinked threads
/*****************************************************/
static void* _unlinked_behavior (void *param)
{
   /*the current thread is the parameter */
   ft_thread_t me = (ft_thread_t)param;
		    ...
		    void *args)
{
   ...
   thread->run          = runnable;
   // create the pthread 
   if (OK == pthread_create (&thread->pthread,NULL,_unlinked_behavior,thread))
       thread->well_created = FT_MAGIC;
   return thread;
}
...
{
   return (thread->signal) ?_get_event_scheduler (thread->signal) : NULL;
}
...
{
   return (ft_thread_t)pthread_getspecific (_self_key);
}

ft_scheduler_t ft_thread_scheduler (void)
{
   ft_thread_t self = (ft_thread_t)pthread_getspecific (_self_key);
...
static void _signal_term_if_possible (ft_thread_t thread)
{
   ...
   if (thread->signal) _event_generate (thread->signal);
}
...
/****************************************************/
static void _reinit_thread (ft_thread_t thread)
{
   thread->timer_cell = NULL;
   thread->awaited_event = NULL;
   thread->event_cell = NULL;
...
void _unregister_in_timer (ft_thread_t thread)
{
   item_cell_t cell = thread->timer_cell;
   if (cell) {
      _extract_from_timer (thread->scheduler,cell);
      free (cell);
   }
   _reinit_thread (thread);
}

void _unregister_in_event (ft_thread_t thread)
{
   item_cell_t cell = thread->event_cell;
   ft_event_t event = thread->awaited_event;
   if (cell && event) {
      _extract_from_event (event,cell);
      free (cell);
   }
   _reinit_thread (thread);
}
...
/****************************************************/
int ft_thread_is_linked (ft_thread_t thread){
   return thread->is_linked;
}
