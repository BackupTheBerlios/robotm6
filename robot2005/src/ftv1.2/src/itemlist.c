...
{
   void                *item;
   struct item_cell_st  *struct_previous; // should never change
   struct item_cell_st  *struct_next;     // should never change   
   struct item_cell_st  *previous;        // to previous unmasked cell   
   struct item_cell_st  *next;            // to next unmasked cell
   char                 masked;
};
...
struct item_list_st
{
   ...
   struct item_cell_st  *first;
   struct item_cell_st  *last;
   struct item_cell_st  *end;
};
...
{
   item_cell_t res = (item_cell_t)mymalloc (sizeof (struct item_cell_st));
...
{
   item_list_t res = (item_list_t)mymalloc (sizeof (struct item_list_st));
   if (res == NULL) return NULL;
...
item_cell_t _append_item_list (item_list_t list,void *item)
{
   item_cell_t newI = _make_item_cell (item);
   if (newI == NULL) return NULL;
   if (0 == list->length) {
      list->first = list->last = newI;
   } else {
      list->last->next = newI;
      newI->previous = list->last;
      list->last = newI;
   }
   // list end
   if (list->end) list->end->struct_next = newI;
   newI->struct_previous = list->end;
   list->end = newI;
   // length
   list->length++;
   return newI;
...
int _concat_item_list (item_list_t target,item_list_t list2)
{
   if (list2 != NULL && list2->length != 0) {
      ...
      while (cell != NULL) {
	 item_cell_t next = cell->next;
	 if (_append_item_list (target,cell->item) == NULL) return EBADMEM;
	 cell = next;
      }
   }
   return OK;
...
int _add_item_list (item_list_t list,void *item)
{
   ...
   return _append_item_list (list,item) != NULL ? OK : EBADMEM;
}
...
void _extract (item_list_t list,item_cell_t cell)
{
#ifdef VERIF
...
void _reinsert_cell (item_list_t list,
		     item_cell_t to_insert,
		     int preserve_order)
{
   item_cell_t prev;
#ifdef VERIF
   ...
   prev = preserve_order ? previous_unmasked (to_insert) : list->last;
