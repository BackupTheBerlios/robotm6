...
void             _decr_thread_deadline      (ft_thread_t);
...
void             _set_thread_awaited_event  (ft_thread_t,ft_event_t);
void             _set_thread_event_cell     (ft_thread_t,item_cell_t);
void             _set_thread_timer_cell     (ft_thread_t,item_cell_t);
...
void             _unregister_in_timer       (ft_thread_t);
void             _unregister_in_event       (ft_thread_t);

/****************************************************************/
#define _POST_EVENT(event) \
   _register_in_event (event)

#define _POST_TIMER(sched) \
   _register_in_timer (sched)

#define _CONTROL_TIMEOUT(deadline) \
   if (deadline <= 0) return ETIMEOUT
